...
export function WorkGallery({
  ...
  return (
    <section className="mt-12">
          ...
          <div className="flex gap-1.5">
            {[-1, 1].map((dir) => (
              <button
                ...
                className="rounded-full border border-zinc-200 p-1.5 text-zinc-500 transition-colors hover:border-zinc-300 hover:text-zinc-900 dark:border-zinc-800 dark:hover:border-zinc-700 dark:hover:text-zinc-100"
              >
                {dir < 0 ? (
      ...
      <div
        ref={railRef}
        className="-mx-4 flex snap-x snap-mandatory gap-4 overflow-x-auto px-4 pb-2 sm:mx-0 sm:px-0 [scrollbar-width:thin]"
      >
        {images.map((img, i) => (
          <button
            key={img.src}
            type="button"
            onClick={() => setIndex(i)}
            className="group relative aspect-16/10 w-[86%] shrink-0 snap-start overflow-hidden rounded-xl border border-zinc-200 bg-zinc-50 transition-colors hover:border-zinc-300 sm:w-[calc(50%-0.5rem)] dark:border-zinc-800 dark:bg-zinc-900 dark:hover:border-zinc-700"
      ...
      {open ? (
        ...
        >
          <div className="flex items-center justify-between gap-4 px-4 py-3 text-zinc-300">
            ...
            <span className="min-w-0 flex-1 truncate text-center text-xs text-zinc-400">
              {images[index ?? 0].alt}
            </span>
          ...
          >
            <div
              className={cn(
                "absolute inset-4 transition-transform duration-300 ease-out sm:inset-8",
